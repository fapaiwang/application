...
class EstateServer
{
    ...
    public function estate_list($area,$price,$sort,$keyword,$limit=30,$type,$years){
        $info = $this->search($area,$price,$sort,$type,$years);
        $where = $info["data"];
    ...
    public function search($area,$price,$sort,$type,$years){
        $param['area']       = $area;
        $param['price']      = $price;
        $param['sort']       = $sort;//排序
        $param['years']       = $years;//楼龄
        if ($type){
    ...
    public function estateDealHouse($estate_id,$limit=0){
        $jilu1 =  model('transaction_record')
            ->where('estate_id',$estate_id)
            ->cache("transaction_record_".$estate_id.$limit,84000);
